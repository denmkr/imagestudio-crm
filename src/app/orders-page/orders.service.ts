...
export class OrdersService {
  ...
  getOrdersBySearch(search: string) {
  ...
  updateOrderStatusByOrderId(eventId: string, id: string) {
  ...
  editOrderById(id: string, deadline: string, doer_id: string, counterparty_id: string, comment: string, orders_positions: Array<any>, documents: Array<any>) {
    ...
    this.http.put('http://backend-crm.imagestudio.su/api/v1/orders/' + id, order).subscribe(
      res => { console.log(res) },
      err => { console.log(err) }
    );
  }

  createNewOrder(deadline: string, doer_id: string, counterparty_id: string, comment: string, orders_positions: Array<any>, documents: Array<any>) {
  ...
  getColumnOrdersByParams(status: string, search: string, user_id: string) {
    let httpParams = new HttpParams();
    ...
    .map(result => {
      ...
      result.orders.map(order => {
        ...
        if (order.counterparty == null) order.counterparty = {
          organization: {
            name: ""
          }
        }
          ...
          item.available_events.map(available_event => {
            switch (available_event) {
              ...
              case "put_in_wait":
                ...
                break;
              case "calculate":
                item_events.push({ name: "Расчет", id: "calculate" });
                break;
              case "set_ordered":
  ...
  getOrdersByParams(status: string, search: string, page: string, user_id: string) {
